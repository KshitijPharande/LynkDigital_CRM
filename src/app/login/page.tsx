...
function LoginForm() {
  ...
  return (
    <div className="max-w-md w-full relative z-10 space-y-6">
      ...
      {isTimeout && (
        <div className="p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-300 text-xs flex items-center gap-2.5 animate-fadeIn">
          <Clock className="w-4 h-4 text-amber-400 shrink-0" />
          <span>
            You have been logged out after <strong>15 minutes of inactivity</strong> for security.
          </span>
        </div>
      )}
...
export default function LoginPage() {
  return (
    <div className="min-h-screen bg-dark-bg flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <Suspense fallback={<div className="text-xs text-dark-muted">Loading workspace...</div>}>
        <LoginForm />
      </Suspense>
    </div>
  );
